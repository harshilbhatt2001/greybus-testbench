 ...
 * negative errno.
 */
static int identify_descriptor(struct interface *intf,
                               struct greybus_descriptor *desc, size_t size)
{
    ...
    switch (desc_header->type) {
        ...
        default:
            pr_error("invalid descriptor type (%hhu)\n", desc_header->type);
            return -EINVAL;
    }

    if (desc_size < expected_size) {
        pr_error("%d descriptor too small (%zu < %zu)\n",
    ...
    if (desc_size != expected_size) {
        pr_error("%d descriptor size mismatch, expected - %zu, actual - %zu)\n",
...
bool manifest_parse(struct svc *svc, int intf_id, void *data, size_t size)
{
    ...
    if (manifest_size != size) {
        pr_error("manifest size mismatch %zu != %hu\n",
                    size, manifest_size);
        return false;
    }
    ...
    if (header->version_major > GREYBUS_VERSION_MAJOR) {
        pr_error("manifest version too new (%hhu.%hhu > %d.%d)\n",
...
struct greybus_descriptor *manifest_get_descriptor(struct interface *intf,
...
int cport_get_protocol(struct interface *intf, uint16_t cport_id)
{
    ...
    if (intf->intf_id == 0 && cport_id == GB_SVC_CPORT_ID)
