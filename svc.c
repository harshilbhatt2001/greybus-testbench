...
int svc_get_next_intf_id(struct svc *s)
{
    ...
    start_again:
    TAILQ_FOREACH(intf, &s->interfaces, intf_node) {
        if (intf_id == intf->intf_id) {
            intf_id++;
            goto start_again;
        }
    }
...
int svc_request_send(uint8_t type, uint8_t intf_id)
{
    ...
    switch (type) {
        ...
        case GB_SVC_TYPE_INTF_RESET:
            ...
            break;
        default:
            pr_error("svc operation type %02x not supported\n", type);
            return -EINVAL;
    }
                               ...
                               size_t tsize)
{
    ...
    switch (oph->type) {
        case GB_SVC_TYPE_PROTOCOL_VERSION:
            pr_error("%s(): Protocol Version request not supported\n",
                        __func__);
            break;
        case GB_SVC_TYPE_INTF_DEVICE_ID:
            svc_dev_id = &op_req->svc_intf_device_id_request;

            pr_debug("SVC assign device id (%hhu %hhu) response\n",
        ...
        case GB_SVC_TYPE_CONN_CREATE:
            ...
            pr_debug("SVC conn create request (%hu %hu):(%hu %hu) response\n",
                        ap_intf_id, ap_cport_id, mod_intf_id, mod_cport_id);

            intf = interface_get_by_id(svc, mod_intf_id);
            if (!intf) {
                pr_error("SVC No interface: %hu\n", mod_intf_id);
                break;
            }

            conn = allocate_connection(intf, mod_cport_id,
                                       ap_cport_id);
            if (!conn) {
                pr_error("Failed to allocate conn: (%hu %hu):(%hu %hu)\n",
                            ap_intf_id, ap_cport_id, mod_intf_id,
                            mod_cport_id);
                return -ENOMEM;
            }

            set_connection_protocol(conn, mod_cport_id);
        ...
        case GB_SVC_TYPE_CONN_DESTROY:
            ...
            pr_debug("SVC conn destroy request (%hu %hu):(%hu %hu) response\n",
                        ap_intf_id, ap_cport_id, mod_intf_id, mod_cport_id);

            conn = _get_connection(ap_cport_id);
        ...
        case GB_SVC_TYPE_DME_PEER_GET:
            ...
            pr_debug("SVC dme peer get (%hhu %hu %hu) request\n",
                        dme_get_request->intf_id, dme_get_request->attr,
                        dme_get_request->selector);
            pr_debug("SVC dme peer get (%hu %u) response\n",
        ...
        case GB_SVC_TYPE_ROUTE_DESTROY:
            ...
            pr_debug("SVC route destroy request (%hhu:%hhu) response\n",
                        svc_route_destroy->intf1_id,
                        svc_route_destroy->intf2_id);
            break;
        case GB_SVC_TYPE_PING:
            pr_debug("SVC ping request response\n");
            break;
        ...
        case GB_SVC_TYPE_INTF_RESET:
        default:
            pr_error("%s: Request not supported (%d)\n", __func__,
                        oph->type);
            return -EINVAL;
    }
                                ...
                                void *rbuf, size_t rsize)
{
    ...
    if (cport_id != GB_SVC_CPORT_ID || cport_id != hd_cport_id) {
        pr_error("%s: Error: cport-id-mismatch (%d %d %d)", __func__,
    ...
    switch (oph->type & ~OP_RESPONSE) {
        case GB_SVC_TYPE_PROTOCOL_VERSION:
            pr_debug("%s: Version major-%d minor-%d\n", __func__,
            ...
            if (ret) {
                pr_error("%s: Failed to send svc hello request (%d)\n",
                            __func__, ret);
                return ret;
            }
            break;
        case GB_SVC_TYPE_SVC_HELLO:
             ...
             * hotplug.
             */
            pr_debug("AP's SVC cport is ready now\n");
//            ret = inotify_start(svc, hotplug_basedir);
//            if (ret < 0)
//                pr_error("Failed to start inotify thread\n");
            break;
        case GB_SVC_TYPE_MODULE_REMOVED:
            break;
        case GB_SVC_TYPE_MODULE_INSERTED:
        case GB_SVC_TYPE_INTF_RESET:
            break;
        default:
            pr_error("%s: Response not supported (%d)\n", __func__,
...
int svc_handler(struct connection *conn, void *rbuf,
                size_t rsize, void *tbuf, size_t tsize)
{
    struct op_msg *op = rbuf;
    struct gb_operation_msg_hdr *oph = &op->header;
    uint16_t cport_id = conn->cport_id;
    uint16_t hd_cport_id = conn->hd_cport_id;
...
int svc_init(void) {
    ...
    TAILQ_INIT(&svc->interfaces);

    svc->intf = allocate_interface(svc, intf_id);
    if (!svc->intf) {
        pr_error("%s(): Cannot allocate interface %d\n", __func__, intf_id);
        return -ENOMEM;
    }

    conn = allocate_connection(svc->intf, GB_SVC_CPORT_ID, GB_SVC_CPORT_ID);
    if (!conn) {
        pr_error("%s(): Cannot create connection interface %d cport %d\n", __func__, intf_id, GB_SVC_CPORT_ID);
        return -ENOMEM;
    }

    set_connection_protocol(conn, GB_SVC_CPORT_ID);
